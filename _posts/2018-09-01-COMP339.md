...
* e.g. {% raw %}$$\sum_{i=0}^k F_i^2=F_k\times F_{k+1}\text{, for all }k\geq1$${% endraw %} by Induction
    ...
    2. Inductive Hypothesis:
        ...
        Assume {% raw %}$$\sum_{i=0}^k F_i^2=F_k\times F_{k+1}\text{, for all }k\geq1$${% endraw %}, for all {% raw %}$$k=1,2,\dots,n\text{; that is, }k\leq n$${% endraw %}
    3. Inductive Step:
    ...
    4. Proof:
        ...
        {% raw %}$$
        \begin{align*}
        \sum_{i=0}^{n+1} F_i^2 & = \sum_{i=0}^{n} F_i^2 + F+{n+1}^2 \\
        & = F_n\times F_{n+1} + F+{n+1}^2 \\ 

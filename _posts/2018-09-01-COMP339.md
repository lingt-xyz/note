...
3. CHAPTER: four letters word is {% raw %}$$7\times6\times5\times4$${% endraw %}
4. Put A, B, C into 10 numbered boxes
    1. At most one per box: {% raw %}$$10\times9\times8=P(10,3)$${% endraw %}
    2. If boxes can get more than one: {% raw %}$$10\times10\times10$${% endraw %}
5. 3 Objects into 10 numbered boxe, at most one per box:
...
6. How many 4 digital decimal numbers:
    1. If allow {% raw %}$$0$${% endraw %}: {% raw %}$$10\times10\times10\times10$${% endraw %}
    2. If allow {% raw %}$$0$${% endraw %} but no repeated digits: {% raw %}$$10\times9\times8\times7$${% endraw %}
    3. If no leading {% raw %}$$0$${% endraw %} and no repeated digits: {% raw %}$$9\times9\times8\times7$${% endraw %}
7. How many 5 digital numbers:
    1. Must contain {% raw %}$$0$${% endraw %}: 
    ...
    2. Must contain {% raw %}$$0$${% endraw %} and {% raw %}$$1$${% endraw %}: {% raw %}$$10^5-9^5-9^5\color{red}{+8^5}$${% endraw %}
8. Choose a committe of 3 from 80 students:
...
9. Choose a committe of 6. 3 of them are from 60 cs students and 3 are of them from 20 math students:
     {% raw %}$$\binom{60}{3}\times\binom{20}{3}$${% endraw %}
10. 6 TAs, assign each TA to one time slot (A: 1-2pm; B: 2-3pm; C:3-4pm), and every time slot should have 2 TAs:
    1. M1: {% raw %}$$\binom{6}{2}\times\binom{4}{2}\times\binom{2}{2}$${% endraw %}
    2. M2: think it differently. Because every TA should be assigned one time slot, so the first time slot has 6 choices, and the second time slot has 5 choices, and so on. It becomes to an anagram problem.

...
* Choose a committe of *6*; *3* of them are from *60* cs students and *3* are of them from *20* math students:
...
* Theorem 1.1 {% raw %}$$The Binomial Theorem$${% endraw %}. If {% raw %}$$x$${% endraw %} and {% raw %}$$y$${% endraw %} are variables and {% raw %}$$n$${% endraw %} is a positive integer, then
    ...
    Proof: Take {% raw %}$$n=4$${% endraw %} for example.
...
* Theorem 8.1 The Principle of Inclusion and Exclusion. Consider a set {% raw %}$$S\text{, with }\mathbf{card}(S)=N$${%endraw%}, and conditions {%raw%}$$c_i, 1\leqslant i\leqslant t$${%endraw%}, each of which may be satisfied by some of the elements S. The number of elements of S that satisfy *none* of the conditions {%raw%}$$c_i, 1\leqslant i\leqslant t$${%endraw%}, is denoted by {%raw%}$$\bat N=N(\bar c_1 \bar c_2 \bar c_3 \dots \bar c_t)$${%endraw%} where

{% raw %}$$

\begin{align}
\bar N = & N-[N(c_1)+N(c_2)+N(c_3)+\dots + N(c_t)] \\
...
\bar N = + (-1)^t N(c_1 c_2 c_3 \dots c_t)
\end{align}
,
$${% endraw %}

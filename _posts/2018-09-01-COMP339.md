...
* <green>Corollary</green> 8.1 Under the hypotheses of Theorem 8.1, the number of elements in {% raw %}$$S$${%endraw%} that satisfy at least one of the condition {% raw %}$$c_i\text{, where }1\leqslant i\leqslant t$${%endraw%}, is given by {% raw %}$$N(c_1\text{ or }c_2\text{ or }\dots\text{ or } c_t)=N-\overline{N}$${%endraw%}

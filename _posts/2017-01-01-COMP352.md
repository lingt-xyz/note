...
{% raw %}$$
f(n)\leqslant cg(n)\text{ for }n\geqslant n_0
$${% endraw %}


* If {% raw %}$$ f(n)$${% endraw %} is {% raw %}$$ O(g(n))$${% endraw %} then it is bounded above (cannot get bigger than) some constant times g(n).

...
* Pseudocode
    ...
    * Hides program design issues.
    ...
    Input\text{ array }A\text{ of }n\text{ integers}\\
    Output\text{ maximum element of }A\\\\
    currentMax ← A[0]\\
    \text{for }i ← 1\text{ to }n − 1\text{ do}\\
        \text{if }A[i] > currentMax\text{ then }\\
            currentMax ← A[i]\\
...
Input\text{ Nonnegative integer }k\\
Output\text{ The }k^{th}\text{ Fibonacci number }F_k\\\\
\text{if }k \leqslant 1\text{ then }\\
    \text{return }k\\
\text{else }\\
\text{return }binaryFib(k - 1) + binaryFib(k - 2)\\
\end{align}
...
Input\text{ Nonnegative integer }k\\
Output\text{ Pair of Fibonacci numbers }(F_k, F_{k-1})\\\\
\text{if }k=1\text{ then }\\
    \text{return }(k,0)\\
\text{else }\\

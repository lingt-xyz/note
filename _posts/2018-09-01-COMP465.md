...
1. If {% raw %}$$ f(n)=O(n^{\log_b^{a-\epsilon}}) \text{ for some constant } \epsilon>0\text{, then }T(n)=\Theta(n^{\log_b^a}) $${% endraw %}.
2. If {% raw %}$$ f(n)=\Theta(n^{\log_b^{a}})\text{, then }T(n)=\Theta(n^{\log_b^a} \log n) $${% endraw %}.
3. If {% raw %}$$ f(n)=\Omega(n^{\log_b^{a+\epsilon}}) \text{ for some constant }\epsilon>0 \text{, and if}af(n/b)\leq cf(n)\text{ for some constant }c<1\text{ and all sufficiently large }n\text{ then }T(n)=\Theta(f(n)) $${% endraw %}.

...
3. If {% raw %}$$ f(n)=\Omega(n^{\log_b^{a+\epsilon}})$${% endraw %} for some constant {% raw %}$$\epsilon>0 $${% endraw %}, and if {% raw %}$$af(n/b)\leq cf(n)$${% endraw %} for some constant {% raw %}$$c<1$${% endraw %} and all sufficiently large {% raw %}$$n$${% endraw %} then {% raw %}$$T(n)=\Theta(f(n)) $${% endraw %}.

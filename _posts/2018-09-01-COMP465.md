...
1. If {% raw %}$$ f(n)=O(n^{\log_b^{a-\epsilon}})$${% endraw %} for some constant {% raw %}$$ \epsilon>0$${% endraw %}, then {% raw %}$$T(n)=\Theta(n^{\log_b^a}) $${% endraw %}.
2. If {% raw %}$$ f(n)=\Theta(n^{\log_b^{a}})$${% endraw %}, then {% raw %}$$T(n)=\Theta(n^{\log_b^a} \log n) $${% endraw %}.
3. If {% raw %}$$ f(n)=\Omega(n^{\log_b^{a+\epsilon}})$${% endraw %} for some constant {% raw %}$$\epsilon>0 $${% endraw %}, and if {% raw %}$$af(n/b)\leq cf(n)$${% endraw %} for some constant {% raw %}$$c<1$${% endraw %} and all sufficiently large {% raw %}$$n$${% endraw %}, then {% raw %}$$T(n)=\Theta\bigl(f(n)\bigr) $${% endraw %}.

* e.g.:
    1. Master theorem case 1: {% raw %}$$ T(n) = 7T(n/2) + n^2 $${% endraw %}

        {% raw %}$$
        a=7, b=2, f(n)=n^2\\
        n^{\log_b^a}=n^{\log_2^7}=n^{2.81}\\
        f(n)=O(n^\log_b^{a-0.81}, \epsilon=0.81\\
        \Rightarrow T(n)=\Theta(n^{\log_b^a})=\Theta(n^{2.81})
        $${% endraw %}

    2. Master theorem case 3: {% raw %}$$ T(n) = 7T(n/3) + n^2 $${% endraw %}

...
* Square-Matrix_Multiply:

    Running time: {% raw %}$$T(n)=\Theta (n^3)$${% endraw %}
...
* Lower bound
    ...
    Running time: {% raw %}$$T(n)=\Omega (n^2)$${% endraw %}
...
* e.g.: {% raw %}$$T(n)=2T(\lfloor n/2\rfloor)+n$${% endraw %}
    ...
    2. Prove:
        Assume {% raw %}$$T(\lfloor{k}\rfloor)\leq c\lfloor{k}\rfloor\log{\lfloor{k}\rfloor}, k\leq n\text{, that is,}T(\lfloor{n/2}\rfloor)\leq c\lfloor{n/2}\rfloor\log{\lfloor{n/2}\rfloor}$${% endraw %}
...
* e.g. {% raw %}$$\sum_{i=0}^k F_i^2=F_k\times F_{k+1}\text{, for all }k\geq1$${% endraw %} by Induction
    ...
    3. Inductive Step: {% raw %}$$\sum_{i=0}^{n+1} F_i^2=F_{n+1}\times F_{(n+1))+1}$${% endraw %}
        Must prove 

...
2. If {% raw %}$$ f(n)/{n^{\log_b{a}}}\in \Theta({\log^\epsilon{n}})$${% endraw %} for some constant {% raw %}$$ \epsilon\geq 0$${% endraw %}, then {% raw %}$$T(n)=\Theta(n^{\log_b{a}} {\log^{\epsilon+1}{n}}) $${% endraw %}.
3. If {% raw %}$$ f(n)/{n^{\log_b{a}}}\in \Omega(n^{\epsilon})$${% endraw %} for some constant {% raw %}$$\epsilon>0 $${% endraw %}, and if {% raw %}$$f(n/b)\leq cf(n)$${% endraw %} for some constant {% raw %}$$c<1$${% endraw %} and all sufficiently large {% raw %}$$n$${% endraw %}, then {% raw %}$$T(n)=\Theta\bigl(f(n)\bigr) $${% endraw %}.
